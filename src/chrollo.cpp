...
void hook_dump() {
	...
	int hook_addr = -1;
	show_message(std::to_string(lua_shared_size));
	// `patch` is actually our signature as well
	for (byte* addr = (byte*)luaJIT_version_2_0_4 + 0x1AC8; addr  < (byte*)luaJIT_version_2_0_4 + 0x1AC8 + patchSize; addr++) {
		int m = 0;
